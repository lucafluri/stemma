...
export async function runImportAi(mode) {
  // mode: 'image' (default if image loaded) | 'text'
  const useText = mode === 'text' || (!state._importImageData && mode !== 'image');
  ...
  } else if (!state._importImageData) {
  ...
  try {
    const persons = await _aiParseContent(apiKey, model, useText ? textVal : null, useText ? null : state._importImageData);
...
export function parseImportText() {
  ...
  if (!state._importActions.length) {
...
export function _renderImportReview() {
  ...
  const sorted = [...state._importActions].sort((a, b) => (a._sourceIdx ?? 0) - (b._sourceIdx ?? 0));
...
export function _importApproveAll() { state._importActions.forEach(a => a.status = 'approved'); _renderImportReview(); }

export function _importSkipAll()    { state._importActions.forEach(a => a.status = 'skipped');  _renderImportReview(); }

export function _importResetAll()   { state._importActions.forEach(a => a.status = 'pending');  _renderImportReview(); }
...
export function openMatchDialog(actionId) {
  const action = state._importActions.find(a => a.id === actionId);
  if (!action) return;
...
export async function _aiParseContent(apiKey, model, text, imageData) {
...
export function _aiMapToPersons(arr) {
  return arr.map(p => ({
